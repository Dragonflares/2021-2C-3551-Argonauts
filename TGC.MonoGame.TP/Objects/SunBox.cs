...
namespace TGC.MonoGame.TP.Objects
{
    public class SunBox
    {
        ...
        public SunBox(Model model, Effect effect) : this(model, effect, 8000)
        {
        }
        ...
        public SunBox(Model model, Effect effect, float size)
        {
            Model = model;
            Effect = effect;
            Size = size;
        ...
        public void Draw(Matrix view, Matrix projection, Vector3 positionSun)
        {
                ...
                foreach (var mesh in Model.Meshes)
                {
                    ...
                    foreach (var part in mesh.MeshParts)
                    {
                        ...
                        part.Effect.Parameters["World"]?.SetValue(
                            Matrix.CreateScale(Size) * Matrix.CreateTranslation(positionSun));
